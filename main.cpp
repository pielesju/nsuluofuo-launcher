...
std::string generateParameter(Parameter *parameter) {
    return "-P" + parameter->key + "=" + parameter->value;
}
...
void show_gui() {
    ...
    gtk_label_set_markup(GTK_LABEL(header_label),
        "<span font_desc=\"serif 40\">Nsuluofuo</span>");
    gtk_label_set_justify(GTK_LABEL(header_label), GTK_JUSTIFY_CENTER);

    GtkWidget *header2_label = gtk_label_new("- Java Edition -");
    gtk_label_set_markup(GTK_LABEL(header2_label),
        "<span font_desc=\"serif 20\">- Java Edition -</span>");
    ...
    gtk_widget_set_visible(settings_content_box, false);
    ...
    gtk_widget_set_visible(start_content_box, false);
    ...
    g_signal_connect(start_button, "clicked",
        G_CALLBACK(set_start_state), NULL);
    ...
    g_signal_connect(settings_button, "clicked",
        G_CALLBACK(set_settings_state), NULL);
    ...
    g_signal_connect(setting_button, "clicked",
        G_CALLBACK(set_settings_state), NULL);
...
int main(int argc, char *argv[]) {
    ...
    app = gtk_application_new("org.cofezuwo.nsuluofuo.launcher",
